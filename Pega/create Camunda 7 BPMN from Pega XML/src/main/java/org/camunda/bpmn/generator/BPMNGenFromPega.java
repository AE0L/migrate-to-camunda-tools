...
public class BPMNGenFromPega {
    public static void main(String[] args) throws Exception {
        //try {
            ...
            // Create hash map of Pega elements to search for (maps pxObjClass -> BPMN element type)
            ...
            pegaElementsMap.put("Data-MO-Event-Start-StartScreenFlow", bpmnElement);
            ...
            // For the diagram, a diagram and a plane element needs to be created
            ...
            XPathExpression searchRequest;
            XPath xpath = XPathFactory.newInstance().newXPath();

            // Report any pxObjClass values found in //pyShapes/rowdata that are not mapped
            searchRequest = xpath.compile("//pyShapes/rowdata/pxObjClass");
            NodeList allShapeClasses = (NodeList) searchRequest.evaluate(doc, XPathConstants.NODESET);
            for (int i = 0; i < allShapeClasses.getLength(); i++) {
                String cls = allShapeClasses.item(i).getTextContent();
                if (!pegaElementsMap.containsKey(cls)) {
                    System.out.println("WARNING: Unrecognized pxObjClass '" + cls + "' - this shape will be skipped. Add it to pegaElementsMap in the source code.");
                }
            }

            // Find shapes by pxObjClass (works for both old and new Pega XML formats)
            Iterator iter = pegaElementsMap.keySet().iterator();
            while (iter.hasNext()) {
                String key = (String) iter.next();
                bpmnElement = (PegaToBPMNElement) pegaElementsMap.get(key);
                searchRequest = xpath.compile("//pyShapes/rowdata[pxObjClass/text() = '" + key + "']");
                NodeList shapeNodes = (NodeList) searchRequest.evaluate(doc, XPathConstants.NODESET);
                for (int i = 0; i < shapeNodes.getLength(); i++) {
                    Node parentNode = shapeNodes.item(i);
                    ...
                    FlowNodeInfo fni = new FlowNodeInfo(element.getAttributeValue("id"),
                        Double.valueOf(xCoordList.item(0).getTextContent()),
                        Double.valueOf(yCoordList.item(0).getTextContent()),
                        newX, newY, bpmnElement.getType().toString(),
                        bpmnElement.getHeight(), bpmnElement.getWidth());
            ...
            // Find end events listed in pyEndingActivities (supplemental, skips duplicates)
            ...
            for (int i = 0; i < endingActivitiesList.getLength(); i++) {
                ...
                if (flowNodesMap.containsKey(endElement.getTextContent())) {
                    continue;
                }
                ...
                if (endingActivityList.getLength() == 0) {
                    System.out.println("WARNING: End activity '" + endElement.getTextContent() + "' not found in shapes, skipping.");
                    continue;
                }
                ...
                FlowNodeInfo fni = new FlowNodeInfo(element.getAttributeValue("id"),
                    Double.valueOf(xCoordList.item(0).getTextContent()),
                    Double.valueOf(yCoordList.item(0).getTextContent()),
                    newX, newY, EndEvent.class.toString(), 36d, 36d);
                searchRequest = xpath.compile("pyMOId");
                NodeList idList = (NodeList) searchRequest.evaluate(parentNode, XPathConstants.NODESET);
                flowNodesMap.put(idList.item(0).getTextContent(), fni);
            }
            ...
            for(int i=0; i < sequenceList.getLength(); i++) {
                ...
                if (fromFNI == null || toFNI == null) {
                    System.out.println("WARNING: Skipping connector from '" + fromId + "' to '" + toId + "' because " +
                        (fromFNI == null ? "source '" + fromId + "' not in flowNodesMap" : "") +
                        (fromFNI == null && toFNI == null ? " and " : "") +
                        (toFNI == null ? "target '" + toId + "' not in flowNodesMap" : "") +
                        ". This shape may have an unrecognized pyShapeType or may not be a flow node.");
                    continue;
                }
